...
describe("email subject boundary", () => {
  ...
  it("emits the configured Reply-To address through the common Cloudflare dispatch boundary", async () => {
    const send = vi.fn(async (_message: unknown) => ({ messageId: "email_cf_reply_to" }));
    const service = new EmailService(
      {
        MOCK_EXTERNAL_SERVICES: "false",
        EMAIL_FROM: "legacy-contact-4@example.com",
  ...
  it("includes the configured Reply-To address in the HTTP provider payload", async () => {
    const providerFetch = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) =>
      new Response(JSON.stringify({ status: "accepted", id: "email_http_reply_to" }), {
        status: 202,
        headers: { "content-type": "application/json" }
      })
    );
    ...
    try {
      ...
      const body = JSON.parse(String(providerFetch.mock.calls[0][1]?.body));
      expect(body.replyTo).toBe("legacy-contact-7@example.com");
    } finally {
      vi.unstubAllGlobals();
    }
  });
