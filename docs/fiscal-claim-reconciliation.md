...
```bash
node scripts/run-private-wrangler.mjs d1 execute diezmossv-staging-example --env staging --remote --file "$RECONCILIATION_SQL"
node scripts/run-private-wrangler.mjs d1 execute diezmossv-production-example --env production --remote --file "$RECONCILIATION_SQL"
```
