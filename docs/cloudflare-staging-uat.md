...
4. Set staging secrets. Do not commit these values. Generate the bootstrap token with
   ...
   JSON `{"status":"accepted","id":"<provider-id>"}` (or `messageId`) with a non-empty provider ID.
   The raw provider ID is discarded immediately and only its fixed-length `sha256:` digest is stored.
   Only a `4xx` JSON response
