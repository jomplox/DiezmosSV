...
5. Set staging secrets. Do not commit these values. Generate the bootstrap token with
   ...
   JSON `{"status":"accepted","id":"<provider-id>"}` (or `messageId`), where the provider ID is an
   opaque 1–128 character value using only ASCII letters, digits, `_`, and `-`; URLs, email
   addresses, and credential-shaped IDs are rejected. Only a `4xx` JSON response
