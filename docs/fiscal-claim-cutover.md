...
This repository does not perform the cutover. The deployment operator must complete these steps in order for staging and, after approval, production:
...
5. In the same shell, acknowledge the drained state with `export FISCAL_CUTOVER_QUIESCED=1`, then run the environment's `cf:migrate` command and immediately run its `cf:deploy` command.
