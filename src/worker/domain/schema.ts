import Ajv, { type ErrorObject } from "ajv";
import addFormats from "ajv-formats";
import cdeSchema from "../../../DTE/svfe-json-schemas/v2/fe-cd-v2.json";
import invalidacionSchema from "../../../DTE/svfe-json-schemas/v3/invalidacion-schema-v3.json";
import contingenciaSchema from "../../../DTE/svfe-json-schemas/v4/contingencia-schema-v4.json";

const ajv = new Ajv({ allErrors: true, strict: false });
addFormats(ajv);

const validators = {
  cde: ajv.compile(cdeSchema),
  invalidacion: ajv.compile(invalidacionSchema),
  contingencia: ajv.compile(contingenciaSchema)
};

export function validateCde(document: unknown): void {
  assertValid("CDE", validators.cde(document), validators.cde.errors);
}

export function validateInvalidacion(document: unknown): void {
  assertValid("Invalidacion", validators.invalidacion(document), validators.invalidacion.errors);
}

export function validateContingencia(document: unknown): void {
  assertValid("Contingencia", validators.contingencia(document), validators.contingencia.errors);
}

function assertValid(label: string, valid: boolean, errors: ErrorObject[] | null | undefined): void {
  if (valid) {
    return;
  }
  const detail = (errors ?? [])
    .slice(0, 8)
    .map((error) => `${error.instancePath || "/"} ${error.message ?? ""}`.trim())
    .join("; ");
  throw new Error(`${label} schema validation failed: ${detail}`);
}
